...
- **Debian, Ubuntu, Mint**: `sudo apt install ./MarkdownStudio-0.12.0-linux-amd64.deb` (apt installs the required system libraries automatically).
- **Fedora, RHEL, openSUSE**: `sudo dnf install ./MarkdownStudio-0.12.0-linux-x86_64.rpm` (or `sudo zypper install` on openSUSE).
...
- Format menu and shortcuts: bold (Ctrl/Cmd+B), italic (I), link (K), inline code (E), strikethrough, headings (Ctrl/Cmd+Alt+1–3), promote/demote heading (Ctrl/Cmd+Alt+= / Ctrl/Cmd+Alt+-), Move Section Up/Down (moves a heading with its text and subsections past its neighbour), lists, task lists, check/uncheck task (Ctrl/Cmd+Enter), quotes, code blocks, tables and footnotes (Insert Footnote, Ctrl/Cmd+Alt+R: numbered `[^n]` reference with its definition added at the end)
...
- Export to PDF (selectable text, clickable links, heading bookmarks, tables, task checkboxes, images, Mermaid diagrams, footnotes) and to Word (.docx) with real Word headings, numbered/bulleted/task lists, tables, code, links, embedded images, Mermaid diagrams as pictures and native Word footnotes
...
- Command palette (Ctrl/Cmd+Shift+P), document outline (drag headings to reorder sections; right-click a heading to copy a link to it or move its section; Alt+Up/Down moves the focused section), and Find in Files across the workspace (Ctrl/Cmd+Shift+F; match case, whole word, regex)
