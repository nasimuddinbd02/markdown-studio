...
const MENUS: { label: string; items: (recent: RecentEntry[]) => Item[] }[] = [
  {
    ...
    items: (recent) => [
      ...
      sep, c("save"), c("saveAs"), c("saveAll"), c("fileHistory"), sep, c("importDocx"), c("importHtml"), sep, c("exportHtml"), c("copyHtml"), c("print"), sep, c("closeTab"), c("closeFolder"), sep, c("settings"),
    ],
  },
  {
    label: "Edit",
