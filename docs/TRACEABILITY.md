...
Status of each [SRS](SRS.md) requirement as of version 0.12.0. **Done** means implemented and tested (automated or manual). **Partial** means some of it is implemented and the gap is noted. **Planned** means it's scheduled for the release named in the SRS.
...
| Level | Where | Count (0.12.0) |
| --- | --- | --- |
| Unit and component | `tests/` (Vitest, Testing Library, jsdom) | 234 tests in 37 files |
| End-to-end and accessibility | `e2e/` (Playwright; axe-core WCAG 2.1 AA audits in light and dark themes) | 24 tests |
